create_analytics_parallel <- function(container,classification_results,b=1) {	
	create_documentSummary <- function(container, scores) {
		testing_codes <- as.numeric(as.vector(container@testing_codes))
		return(
		  cbind(
		    MANUAL_CODE=testing_codes,
		    CONSENSUS_CODE=scores$BEST_LABEL,
		    CONSENSUS_AGREE=scores$NUM_AGREE,
		    CONSENSUS_INCORRECT=testing_codes!=scores$BEST_LABEL,
		    PROBABILITY_CODE=as.numeric(as.vector(scores$BEST_PROB)),
		    PROBABILITY_INCORRECT=testing_codes!=scores$BEST_PROB
		    )
		  )
	}
	...
	create_topicSummary<-function(container, scores){
		...
		return(
		  cbind(
		    TOPIC_CODE=as.numeric(as.vector(topic_codes)),
		    NUM_MANUALLY_CODED=manually_coded,
		    NUM_CONSENSUS_CODED=automatically_coded_label,
		    NUM_PROBABILITY_CODED=automatically_coded_prob,
		    PCT_CONSENSUS_CODED=over_coded_label,
		    PCT_PROBABILITY_CODED=over_coded_prob,
		    PCT_CORRECTLY_CODED_CONSENSUS=correctly_coded_label,
		    PCT_CORRECTLY_CODED_PROBABILITY=correctly_coded_prob
		    )
		  )
	}
	
	if (container@virgin == FALSE) {
		score_summary <- suppressWarnings(create_scoreSummary_parallel(container, classification_results))
		...
		algorithm_summary <- suppressWarnings(as.data.frame(create_precisionRecallSummary_parallel(container, classification_results, score_summary, b)))
		...
		analytics_container <- new("analytics", label_summary=as.data.frame(topic_summary)[,-1], document_summary=as.data.frame(raw_summary), algorithm_summary=as.data.frame(algorithm_summary), ensemble_summary=ensemble_summary)
    } else {
		score_summary <- suppressWarnings(create_scoreSummary_parallel(container, classification_results))
	  ...
	  analytics_container <- new("analytics_virgin", label_summary=as.data.frame(topic_summary)[,-1], document_summary=as.data.frame(raw_summary))
	}
