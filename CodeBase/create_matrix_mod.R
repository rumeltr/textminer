create_matrix_mod<-function(textColumns, language="english", minDocFreq=1, maxDocFreq=Inf, minWordLength=3, maxWordLength=Inf, ngramLength=1, originalMatrix=NULL, removeNumbers=FALSE, removePunctuation=TRUE, removeSparseTerms=0, removeStopwords=TRUE, stemWords=FALSE, stripWhitespace=TRUE, toLower=TRUE, weighting=weightTf){
  ...
  if (stemWords == TRUE && ngramLength == 1){
    control<-append(control,list(stemming=stem_words),after=7)
  }
  ...
  if (!is.null(originalMatrix)){
    ...
    fixed<-as.DocumentTermMatrix(
      cbind(
        matrix[,which(colnames(matrix) %in% colnames(originalMatrix))],
        amat
        ),
      weighting=weighting
      )
