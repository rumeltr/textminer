create_container_corrected<-function(matrix,labels,trainSize=NULL,testSize=NULL,virgin){
  ...
  test_code<-as.factor(labels[testSize])
	...
	if (virgin == FALSE && length(unique(is.na(test_code))) > 1){
	  stop("The data to be classified does not have corresponding codes. To treat this data set as virgin data, set virgin=TRUE.")
	}
