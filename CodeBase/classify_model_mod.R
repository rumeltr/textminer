classify_model<-function(container, model, s=0.01, ...){
  gc()
  ...
  if (pmatch("svm",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("slda",class(model),nomatch=0) > 0){
	...
	} else if (pmatch("LogitBoost",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("classbagg",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("randomForest",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("glmnet",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("tree",class(model),nomatch=0) > 0){
  ...
  } else if (pmatch("nnet",class(model),nomatch=0) > 0){
    ...
    } else if (pmatch("maxent",class(model),nomatch=0) > 0) {
	...
	} else if (pmatch("naiveBayes",class(model),nomatch = 0)>0){
	  naiveBayes_pred<-predict(model,data.frame(as.matrix(container@classification_matrix)))
	  naiveBayes_prob<-predict(model,data.frame(as.matrix(container@classification_matrix)))
	  results_table <- data.frame(as.character(naiveBayes_pred),as.numeric(naiveBayes_prob))
	  colnames(results_table)[1] <- "NAIVEBAYES_LABEL"
	  colnames(results_table)[2] <- "NAIVEBAYES_PROB"
	}
